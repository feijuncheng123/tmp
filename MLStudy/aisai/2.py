...
def to_database_tmp(df, i=0):
    try:
        '''
        conn_string='oracle+cx_oracle://fmsrb:smart@10.180.9.8:1521/rb'
        engine = create_engine(conn_string, echo=False)
        dtype={'CUST_ID':sqlalchemy.types.VARCHAR(),
            'score':sqlalchemy.types.NUMERIC(),
            'create_time':sqlalchemy.types.DateTime(),
            'identity_character':sqlalchemy.types.NUMERIC(),
            'social_relationship':sqlalchemy.types.NUMERIC(),
            'behavior_preference':sqlalchemy.types.NUMERIC(),
            'credit_history':sqlalchemy.types.NUMERIC(),
            'repayment_capacity':sqlalchemy.types.NUMERIC(),
            # 2020.4.1新增
            'CUST_FLAG':sqlalchemy.types.NUMERIC()}
        df.to_sql(
            'CUST_SCORE_INFO_TMP', con=engine, 
            if_exists='append', index=False, dtype=dtype, chunksize=10000,
            schema='fmsrb')
        '''
...
if __name__ == '__main__':
    ...
    try:
        ...
        drop_tmp_table()  # 删除临时表
        logger.info('Finish Drop Tmp Table')
