...
if __name__ == "__main__":
    ...
    centers= [[1, 2,1,1,2], [-1, -1,-2,-1,-2], [1, -1,0,1,0], [-1, 1,1,1,-1]]
    data, y = ds.make_blobs(n_samples=N, n_features=5, centers=centers, cluster_std=[0.5, 0.25, 0.7, 0.5], random_state=2)
    print(data.shape)
    ...
    for i in range(6):
        ...
        #     clrs.append('#%06x' % c)
        plt.subplot(2, 3, i+1)
        clrs = plt.cm.Spectral(np.linspace(0, 0.8, y_unique.size))  #给label为1的点一种颜色，给label为0的点另一种颜色
        print(clrs)
        ...
        x1_min, x2_min,_,_,_ = np.min(data, axis=0)
        x1_max, x2_max,_,_,_ = np.max(data, axis=0)
