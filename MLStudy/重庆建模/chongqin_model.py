...
data[meanCols]=data[meanCols].fillna(data[modeCols].mean().iloc[0])


modelData=data[~data["label"].isna()]
modelData[modeCols]=modelData[modeCols].fillna(modelData[modeCols].mode().iloc[0])
modelData[meanCols]=modelData[meanCols].fillna(modelData[modeCols].mean().iloc[0])
...
gbdt_param_grid={'n_estimators':[70,100,50]}

gbdt=GradientBoostingClassifier(max_depth=10,random_state=12345,verbose=1)

gbdt_GridSearch=GridSearchCV(gbdt,param_grid=gbdt_param_grid,scoring="f1_score",cv=10,n_jobs=1,verbose=3)
gbdt_model=gbdt_GridSearch.fit(train_data,y_train)

model=gbdt.fit(X_train,y_train)
y_pred=model.predict(X_test)
f1=f1_score(y_test,y_pred,average="macro")
submit=model.predict(submitData[modelCols])
