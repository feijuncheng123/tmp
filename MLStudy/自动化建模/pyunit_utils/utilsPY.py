...
def gen_random_time(numberTimes, maxtime=datetime.datetime(2080, 8, 6, 8, 14, 59),
...
def check_models(model1, model2, use_cross_validation=False, op='e'):
    ...
    if isinstance(model1, H2OBinomialModel):  # 2a. Binomial
        ...
        if op == 'e':
            assert f1_1[0][1] == f1_2[0][1], "The first model has an F1 of {0} and the second model has an F1 of " \
                                             "{1}. Expected the first to be == to the second.".format(f1_1[0][1],
                                                                                                      f1_2[0][1])
        elif op == 'g':
            assert f1_1[0][1] > f1_2[0][1], "The first model has an F1 of {0} and the second model has an F1 of " \
                                            "{1}. Expected the first to be > than the second.".format(f1_1[0][1],
                                                                                                      f1_2[0][1])
        elif op == 'ge':
            assert f1_1[0][1] >= f1_2[0][1], "The first model has an F1 of {0} and the second model has an F1 of " \
                                             "{1}. Expected the first to be >= than the second.".format(f1_1[0][1],
        ...
        if op == 'e':
            assert mse1 == mse2, "The first model has an MSE of {0} and the second model has an MSE of " \
        ...
        if op == 'e':
            assert totss1 == totss2, "The first model has an TOTSS of {0} and the second model has an " \
...
def mojo_predict(model, tmpdir, mojoname, glrmReconstruct=False, get_leaf_node_assignment=False):
    ...
    pred_mojo = h2o.import_file(os.path.join(tmpdir, 'out_mojo.csv'),
                                header=1)  # load mojo prediction into a frame and compare
        ...
        for r in range(hr):
            for c in range(hc):
                ...
                if isinstance(hp, numbers.Number):
                    ...
                    assert (math.fabs(hp - pp) < 1e-8) or (math.isnan(hp) and math.isnan(
                        pp)), "Expected munged rows to be the same for row {0}, but got {1}, and {2}".format(r, hp, pp)
                else:
                    assert hp == pp, "Expected munged rows to be the same for row {0}, but got {1}, and {2}".format(r,
                                                                                                                    hp,
                                                                                                                    pp)
...
def test_is_on_hadoop():
    if hasattr(sys.modules["pyunit_utils.pyunit_utils"], '__on_hadoop__'):
        return sys.modules["pyunit_utils.pyunit_utils"].__on_hadoop__
    return False
...
def hadoop_namenode():
    ...
    elif hasattr(sys.modules["pyunit_utils.pyunit_utils"], '__hadoop_namenode__'):
        return sys.modules["pyunit_utils.pyunit_utils"].__hadoop_namenode__
    return None
...
def run_tests(tests, run_in_isolation=True):
    # flatten in case of nested pyunit_utils/test suites
...
def make_random_grid_space(algo, ncols=None, nrows=None):
    ...
    if algo in ["gbm", "rf"]:
        ...
        if algo == "gbm":
            ...
            grid_space['distribution'] = random.sample(
                ['bernoulli', 'multinomial', 'gaussian', 'poisson', 'tweedie', 'gamma'], 1)
        if algo == "rf":
            if random.randint(0, 1): grid_space['mtries'] = random.sample(list(range(1, ncols + 1)),
                                                                          random.randint(2, 3))
        ...
        if random.randint(0, 1): grid_space['max_iterations'] = random.sample(list(range(1, 1000)),
                                                                              random.randint(2, 3))
        ...
        if random.randint(0, 1): grid_space['init'] = random.sample(['Random', 'PlusPlus', 'Furthest'],
                                                                    random.randint(2, 3))
    elif algo == "glm":
        ...
        if grid_space['family'] == "tweedie":
            if random.randint(0, 1):
                grid_space['tweedie_variance_power'] = [round(random.random() + 1, 6) for r in
        ...
        if random.randint(0, 1): grid_space['laplace'] = [round(random.random() + r, 6) for r in
                                                          random.sample(list(range(0, 11)), random.randint(2, 3))]
        if random.randint(0, 1): grid_space['min_sdev'] = [round(random.random(), 6) for r in
                                                           range(random.randint(2, 3))]
        if random.randint(0, 1): grid_space['eps_sdev'] = [round(random.random(), 6) for r in
                                                           range(random.randint(2, 3))]
    elif algo == "pca":
        if random.randint(0, 1): grid_space['max_iterations'] = random.sample(list(range(1, 1000)),
                                                                              random.randint(2, 3))
        if random.randint(0, 1): grid_space['transform'] = random.sample(
            ["NONE", "STANDARDIZE", "NORMALIZE", "DEMEAN", "DESCALE"], random.randint(2, 3))
...
def move_files(dir_path, old_name, new_file, action='move'):
    ...
    with new name (new_file) so that we will be able to re-run the pyunit_utils if we
...
def show_test_results(test_name, curr_test_val, new_test_val):
    ...
    :param curr_test_val: integer representing number of pyunit_utils failed so far before the test specified in test_name
    is executed
    :param new_test_val: integer representing number of pyunit_utils failed after the test specified in test_name is
    executed
...
def assert_H2OTwoDimTable_equal(table1, table2, col_header_list, tolerance=1e-6, check_sign=False, check_all=True,
    ...
    flip_sign_vec = generate_sign_vec(table1, table2) if check_sign else [1] * len(
        table1.cell_values[0])  # correct for sign change for eigenvector comparisons
        ...
        for name_ind1 in randRange1:
            ...
            for name_ind2 in randRange2:
                ...
                if str(table1.cell_values[name_ind1][0]) == str(table2.cell_values[name_ind2][0]):
                    randRange3 = generate_for_indices(
                        min(len(table2.cell_values[name_ind2]), len(table1.cell_values[name_ind1])), check_all,
                        num_per_dim, 1)
                    for indC in randRange3:
                        ...
                        if isinstance(val1, float) and isinstance(val2, float):
                            ...
                            if compare_val_ratio > tolerance:
                                ...
                                print("The first vector is {0} and the second vector is {1}".format(
                                    table1.cell_values[name_ind1], table2.cell_values[name_ind2]))
        ...
        for ind in range(size1):
            if not ((array1[ind] < eps) and (array2[ind] < eps)):
                ...
                if compare_val_h2o_Py > tolerance:  # difference is too high, return false
                    if throwError:
                        assert False, "Array 1 value {0} and array 2 value {1} do not agree.".format(array1[ind],
                                                                                                     array2[ind])
                    else:
                        return False
...
def compare_two_arrays(array1, array2, eps, tolerance, comparison_string, array1_string, array2_string, error_string,
    ...
    :return: if template_is_better = True, return 0 if elements in array1 and array2 are close and 1 otherwise;
             if template_is_better = False, will always return 0 since system under pyunit_utils performs better than
             template system.
    """
...
def extract_comparison_attributes_and_print(model_h2o, h2o_model_test_metrics, end_test_str, want_p_values,
    ...
    :param failed_test_number: integer denote the number of pyunit_utils failed
    ...
    failed_test_number += compare_two_arrays(test1_weight, att1_template, ignored_eps, allowed_diff * 100,
                                             compare_att1_str,
...
def extract_comparison_attributes_and_print_multinomial(model_h2o, h2o_model_test_metrics, family_type, end_test_str,
    ...
    :param ignored_eps: if value < than this value, no comparison is performed
    :param allowed_diff: threshold if exceeded will fail a test
    :param failed_test_number: integer denote the number of pyunit_utils failed so far

    :return: accumulated number of pyunit_utils that have failed so far
    """
            ...
            if (random.uniform(0.0, 1.0) < prob):
                if colInd == sorted_column_indices[0]:
                    if not (math.isnan(frame1[rowInd, colInd])) and not (math.isnan(frame1[rowInd + 1, colInd])):
                        if ascending[colInd]:
                            assert frame1[rowInd, colInd] <= frame1[
                                rowInd + 1, colInd], "Wrong sort order: value at row {0}: {1}, value at " \
                    ...
                    if not (math.isnan(frame1[rowInd, sorted_column_indices[0]])) and not (
                    math.isnan(frame1[rowInd + 1, sorted_column_indices[0]])):
                        if (frame1[rowInd, sorted_column_indices[0]] == frame1[
                            rowInd + 1, sorted_column_indices[0]]):  # meaningful to compare row entries then
                            if not (math.isnan(frame1[rowInd, colInd])) and not (
                            math.isnan(frame1[rowInd + 1, colInd])):
                                if ascending[colInd]:
                                    assert frame1[rowInd, colInd] <= frame1[
                                        rowInd + 1, colInd], "Wrong sort order: value at row {0}: {1}, value at " \
                                                             "row {2}: {3}".format(rowInd, frame1[rowInd, colInd],
                                                                                   rowInd + 1,
                                                                                   frame1[rowInd + 1, colInd])
                                else:
                                    assert frame1[rowInd, colInd] >= frame1[
                                        rowInd + 1, colInd], "Wrong sort order: value at row {0}: {1}, value at " \
                                                             "row {2}: {3}".format(rowInd, frame1[rowInd, colInd],
...
def compare_string_frames_local(f1, f2, prob=0.5):
    ...
    for colInd in range(f1.ncol):
        ...
        for rowInd in range(1, f2.nrow):
            if random.uniform(0, 1) < prob:
                assert temp1[rowInd][colInd] == temp2[rowInd][
                    cname2.index(name1)], "Failed frame values check at row {2} and column {3}! " \
                                          "frame1 value: {0}, frame2 value: " \
                                          "{1}".format(temp1[rowInd][colInd], temp2[rowInd][colInd], rowInd, colInd)
...
def compare_frames_local(f1, f2, prob=0.5, tol=1e-6, returnResult=False):
    ...
    for colInd in range(f1.ncol):
        if (typeDict[frameNames[colInd]] == u'enum'):
            if returnResult:
                result = compare_frames_local_onecolumn_NA_enum(f1[colInd], f2[colInd], prob=prob, tol=tol,
            ...
            if returnResult:
                result = compare_frames_local_onecolumn_NA_string(f1[colInd], f2[colInd], prob=prob,
                                                                  returnResult=returnResult)
                if not (result):
                    return False
            else:
            ...
            if returnResult:
                result = compare_frames_local_onecolumn_NA(f1[colInd], f2[colInd], prob=prob, tol=tol,
                                                           returnResult=returnResult)
                if not (result):
                    return False
            else:
...
def compare_frames_local_onecolumn_NA(f1, f2, prob=0.5, tol=1e-6, returnResult=False):
    if (f1.types[f1.names[
        0]] == u'time'):  # we have to divide by 1000 before converting back and forth between ms and time format
        tol = 10
                ...
                if len(temp1[rowInd]) == 0 or len(temp2[rowInd]) == 0:
                    if returnResult:
                        ...
                        assert len(temp1[rowInd]) == len(temp2[rowInd]), "Failed frame values check at row {2} ! " \
                                                                         "frame1 value: {0}, frame2 value: " \
                                                                         "{1}".format(temp1[rowInd], temp2[rowInd],
                                                                                      rowInd)
                else:
                    ...
                    if returnResult:
                        ...
                        assert diff <= tol, "Failed frame values check at row {2} and column {3}! frame1 value: {0}, frame2 value: " \
                                            "{1} and the difference/max(v1,v2) is {4}.  Column type is {5}".format(v1,
                                                                                                                   v2,
                                                                                                                   rowInd,
                                                                                                                   colInd,
                                                                                                                   diff,
            ...
            if (random.uniform(0, 1) < prob):
                if len(temp1[rowInd]) == 0 or len(temp2[rowInd]) == 0:
                    ...
                    if returnResult:
                        ...
                        assert temp1[rowInd][colInd] == temp2[rowInd][
                            colInd], "Failed frame values check at row {2} and column {3}! frame1 value: {0}, frame2 value: " \
...
def summarizeResult_binomial(h2oPredictD, nativePred, h2oTrainTimeD, nativeTrainTime, h2oPredictTimeD,
    ...
    for ind in range(h2oPredictD.nrow):
        assert abs(
            h2oPredictLocalD[colnames[2]][ind] - nativePred[ind]) < tolerance, "H2O prediction prob: {0} and native " \
                                                                               "XGBoost prediction prob: {1}.  They are " \
                                                                               "very different.".format(
            h2oPredictLocalD[colnames[2]][ind], nativePred[ind])
...
def summarizeResult_regression(h2oPredictD, nativePred, h2oTrainTimeD, nativeTrainTime, h2oPredictTimeD,
                               nativeScoreTime, tolerance=1e-6):
    # Result comparison in terms of time
    ...
    # compare prediction probability and they should agree if they use the same seed
    for ind in range(h2oPredictD.nrow):
        assert abs((h2oPredictLocalD['predict'][ind] - nativePred[ind]) / max(1, abs(h2oPredictLocalD['predict'][ind]),
                                                                              abs(nativePred[ind]))) < tolerance, \
...
def summarizeResult_binomial_DS(h2oPredictD, nativePred, h2oTrainTimeD, nativeTrainTime, h2oPredictTimeD,
    ...
    # Result comparison in terms of time
    print(
        "H2OXGBoost train time with sparse DMatrix is {0}s.  Native XGBoost train time with dense DMtraix is {1}s.\n  H2OGBoost scoring time is {2}s."
...
def compare_weightedStats(model, dataframe, xlist, xname, weightV, pdpTDTable, tol=1e-6):
    ...
    :return:
    '''
    weightStat = manual_partial_dependence(model, dataframe, xlist, xname,
                ...
                if typesDict[colnames[cindex]] == u'time':
                    writeWords = writeWords + \
                                 str(datetime.datetime.fromtimestamp(
                                     float(fdataframe[rowindex][cindex]) / 1000.0)) + ","
